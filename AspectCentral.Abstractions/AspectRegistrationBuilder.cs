// --------------------------------------------------------------------------------------------------------------------
// <copyright file="AspectRegistrationBuilder.cs" company="CBRE">
//   
// </copyright>
//  <summary>
...
namespace AspectCentral.Abstractions
{
    ...
    public class AspectRegistrationBuilder : IAspectRegistrationBuilder
    {
        ...
        /// </summary>
        private static readonly MethodInfo CreateFactoryMethodInfo = typeof(AspectRegistrationBuilder).GetMethod("CreateFactory", BindingFlags.Static | BindingFlags.NonPublic);
        ...
        /// </param>
        public AspectRegistrationBuilder(IServiceCollection services, IAspectConfigurationProvider aspectConfigurationProvider)
        {
        ...
        /// <inheritdoc />
        public IAspectRegistrationBuilder AddAspect(Type aspectFactory, int? sortOrder = null, params MethodInfo[] methodsToIntercept)
        {
            if (aspectFactory == null) throw new ArgumentNullException(nameof(aspectFactory));
            if (!Constants.InterfaceIAspectFactoryType.IsAssignableFrom(aspectFactory))
                throw new ArgumentException($"The {nameof(aspectFactory)} must be a concrete class that implements the {Constants.InterfaceIAspectFactoryType} interface", nameof(aspectFactory));
            ...
            AspectConfigurationProvider.ConfigurationEntries.Last().AddEntry(aspectFactory, sortOrder, methodsToIntercept);
            return this;
        }
        ...
        public IAspectRegistrationBuilder AddService(Type service, Type implementation, ServiceLifetime serviceLifetime)
        {
            ...
            if (!implementation.IsConcreteClass() || !service.IsAssignableFrom(implementation))
                throw new ArgumentException($"The {nameof(implementation)} ({implementation.FullName}) must be a concrete class that implements the {nameof(service)} ({service.Name})");

            var aspectConfiguration = new AspectConfiguration(new ServiceDescriptor(service, implementation, serviceLifetime));
            Services.TryAdd(new ServiceDescriptor(implementation, implementation, serviceLifetime));
            Services.Add(new ServiceDescriptor(service, serviceProvider => InvokeCreateFactory(serviceProvider, aspectConfiguration), serviceLifetime));
        ...
        /// <inheritdoc />
        public IAspectRegistrationBuilder AddService(Type service, Func<IServiceProvider, object> factory, ServiceLifetime serviceLifetime)
        {
            if (service == null) throw new ArgumentNullException(nameof(service));
...
#pragma warning disable S1144 // Unused private types or members should be removed
        private static object CreateFactory<TService>(IServiceProvider serviceProvider, AspectConfiguration aspectConfiguration)
        {
            ...
            Func<IServiceProvider, TService> factory = f =>
                aspectConfiguration.ServiceDescriptor.ImplementationType != null ? (TService)f.GetService(aspectConfiguration.ServiceDescriptor.ImplementationType) : instance;
...
#pragma warning restore S1144 // Unused private types or members should be removed
        ...
        /// The <see cref="object"/>.
        /// </returns>
        private static object InvokeCreateFactory(IServiceProvider serviceProvider, AspectConfiguration aspectConfiguration)
        {
